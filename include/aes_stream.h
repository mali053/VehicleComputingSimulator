...
class StreamAES {
   public:
    ...
    {
        if (iv) {
            delete[] iv;   // Free the memory allocated for iv
            iv = nullptr;  // Avoid dangling pointer
        }
        if (lastBlock) {
            delete[] lastBlock;   // Free the memory allocated for lastBlock
            lastBlock = nullptr;  // Avoid dangling pointer
        }
        if (key) {
            delete[] key;   // Free the memory allocated for key
            key = nullptr;  // Avoid dangling pointer
        }
        if (lastData) {
            delete[] lastData;   // Free the memory allocated for lastData
            lastData = nullptr;  // Avoid dangling pointer
        }
    }
    /** 
     @brief Encrypts the initial block of data using AES in CBC mode.
