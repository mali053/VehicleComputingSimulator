                ...
                unsigned int &paddedLength)
{
    ...
    paddedLength =((originalLength + BLOCK_BYTES_LEN - 1) / BLOCK_BYTES_LEN) * BLOCK_BYTES_LEN;
...
{
    if(chainingMode == ECB) {
            encryptFunctions[chainingMode](in, inLen, key, out, outLen, nullptr, keyLength);
            return;
    }
    unsigned char *iv;
    iv = new unsigned char[16];
    generateRandomIV(iv);
    encryptFunctions[chainingMode](in, inLen, key, out, outLen, iv, keyLength);
                ...
                unsigned char *key, unsigned char *&out, unsigned int &outLen, AESKeyLength keyLength, AESChainingMode chainingMode)
{
    if(chainingMode != ECB){
       ...
       decryptFunctions[chainingMode](in, messageLen, key, out, outLen, in + messageLen, keyLength);

       return;
    }

    decryptFunctions[chainingMode](in, inLen, key, out, outLen, NULL, keyLength);
             ...
             unsigned char *&out, unsigned int &outLen, const unsigned char *iv, AESKeyLength keyLength)
{
    ...
    unsigned char *roundKeys =
        new unsigned char[(aesKeyLengthData[keyLength].numRound + 1) * NUM_BLOCKS * AES_STATE_ROWS];
...
/**
 ...
 @param[out] out Encrypted output data.
 @param[out] outLen Length of encrypted data.
 @param iv Initialization vector (unused in ECB mode).
 @param keyLength AES key length (128, 192, 256 bits).
 */
                ...
                unsigned char *&out, unsigned int &outLen, const unsigned char *iv, AESKeyLength keyLength) 
{
    ...
    for (unsigned int i = 0; i < outLen; i += BLOCK_BYTES_LEN) {
        ...
        xorBlocks(block, in + i, out+i, BLOCK_BYTES_LEN);
...
/**
 Encrypts data using AES in CFB mode. 
 @param in Input data.
 @param inLen Length of input data.
 @param key Encryption key.
 @param[out] out Encrypted output data.
                ...
                unsigned char *&out, unsigned int &outLen, const unsigned char *iv, AESKeyLength keyLength) 
{
    ...
    unsigned char *roundKeys = new unsigned char[(aesKeyLengthData[keyLength].numRound + 1) * NUM_BLOCKS * AES_STATE_ROWS];
    keyExpansion(key, roundKeys, keyLength);
    memcpy(counter, iv, BLOCK_BYTES_LEN);
    for (unsigned int i = 0; i < outLen; i += BLOCK_BYTES_LEN) {
...
/**
 Decrypts data using AES in OFB mode. 
 @param in Encrypted input data.
 @param inLen Length of input data.
 @param key Decryption key.
 @param[out] out Decrypted output data.
